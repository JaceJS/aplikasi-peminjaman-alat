...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($foto_brg)) {        
        ...
        if (move_uploaded_file($tmp_foto, $foto_brg_destination)) {            
            ...
            $query = "UPDATE barang SET nama_brg='$nama_brg', jenis_brg='$jenis_brg', stok_brg='$stok_brg', kode='$kode', foto='$foto_brg' WHERE id_brg='$id_brg'";
        } else {            
			echo "<script>alert('Gagal mengunggah file gambar.')</script>";
			exit();
        }
    } else {        
        $query = "UPDATE barang SET nama_brg='$nama_brg', jenis_brg='$jenis_brg', stok_brg='$stok_brg', kode='$kode' WHERE id_brg='$id_brg'";
    }
